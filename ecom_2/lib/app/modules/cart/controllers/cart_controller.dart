...
class CartController extends GetxController {
  ...
  void updateTotal() {
    ...
    cart.forEach((element) {
      total.value = total.value +
          double.parse(element.property.price!) * element.quantity;
    });
  }
  ...
  void mapCart() {
    ...
    this.cart = cart
        .map((e) => CartItem(
            property: Property.fromJson(e['product']), quantity: e['quantity']))
        .toList();
    updateTotal();
  }

  void addProduct({required Property property, int? quantity}) {
    if (cart
        .any((element) => element.property.propertyId == property.propertyId)) {
      Get.showSnackbar(const GetSnackBar(
        backgroundColor: Colors.red,
    ...
    cart.add(CartItem(property: property, quantity: quantity ?? 1));
    updateTotal();
    updateLocal();
    update();
    Get.showSnackbar(const GetSnackBar(
...
class CartItem {
  ...
  CartItem({required this.property, this.quantity = 1});
}
